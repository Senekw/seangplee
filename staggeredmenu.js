// ============================================================================
// StaggeredMenu — React island (no build step).
// React + htm + gsap load from a CDN via the import map in index.html.
// Replaces the old top bar. To remove: delete this file, staggeredmenu.css,
// and the #staggered-menu-root markup (and gsap from the import map).
//
// Component ported from React Bits (https://reactbits.dev) —
  ...
  return html`
    ...
    >
      <div ref=${preLayersRef} className="sm-prelayers" aria-hidden="true">
        ${prelayerColors.map((c, i) => html`<div key=${i} className="sm-prelayer" style=${{ background: c }} />`)}
      </div>
        ...
        >
          <span ref=${textWrapRef} className="sm-toggle-textWrap" aria-hidden="true">
            <span ref=${textInnerRef} className="sm-toggle-textInner">
              ${textLines.map((l, i) => html`<span className="sm-toggle-line" key=${i}>${l}</span>`)}
            </span>
          </span>
        ...
        <div className="sm-panel-inner">
          <ul className="sm-panel-list" role="list" data-numbering=${displayItemNumbering || undefined}>
            ${items && items.length
              ...
              : html`<li className="sm-panel-itemWrap" aria-hidden="true"><span className="sm-panel-item"><span className="sm-panel-itemLabel">No items</span></span></li>`}
          </ul>
          ${displaySocials && socialItems && socialItems.length > 0 && html`
            <div className="sm-socials" aria-label="Social links">
              <h3 className="sm-socials-title">Elsewhere</h3>
              <ul className="sm-socials-list" role="list">
                ${socialItems.map((s, i) => html`
                  <li key=${s.label + i} className="sm-socials-item">
                    <a href=${s.link} target=${s.external === false ? undefined : '_blank'} rel="noopener noreferrer" className="sm-socials-link">${s.label}</a>
                  </li>`)}
              </ul>
            </div>`}
        </div>
...
/* ----- mount: nav + socials for THIS site --------------------------------- */
const menuItems = [
  { label: 'About Me',       ariaLabel: 'Go to the about section',         link: '#about' },
  { label: 'Research',       ariaLabel: 'Go to research and publications', link: '#research' },
...
const socialItems = [
  { label: 'LinkedIn', link: 'https://www.linkedin.com/in/sean-lee-ab5645383/' },
  { label: 'Email', link: 'mailto:seangplee@gmail.com', external: false }
];

const mount = document.getElementById('staggered-menu-root');
if (mount) {
  ...
  // Close the menu after navigating to an in-page section.
  mount.addEventListener('click', (e) => {
    const link = e.target.closest('a.sm-panel-item');
    if (link) {
      const btn = mount.querySelector('.sm-toggle');
      // If the panel is open (toggle says "Close"), trigger a close.
      const wrapper = mount.querySelector('.staggered-menu-wrapper');
      if (wrapper && wrapper.getAttribute('data-open') !== null && btn) btn.click();
    }
  });
}
